...
DrinkStation::DrinkStation() : WorkStation("DrinkStation") {}
...
void DrinkStation::processAction(ActionType action,
  ...
  case ActionType::PrepareDrink:
    ...
    std::cout << "[DrinkStation] Player " << player.getName()
              << " pregateste o bautura pentru comanda #" << order.getID()
              << "\n";
  ...
  case ActionType::ServeDrink:
    if (!hasPreparedDrink) {
      throw ActionException("DrinkStation: nu exista nicio bautura pregatita "
                            "pentru a fi servita");
    }

    std::cout << "[DrinkStation] Player " << player.getName()
              << " serveste bautura pentru comanda #" << order.getID() << "\n";
