...
// parametric
Player::Player(const std::string &n, float balance)
    : name(n), balance(balance), days(0) {
  if (name.empty()) {
...
void Player::finishOrder() {
  ...
  float timeRatio = static_cast<float>(elapsed.count()) /
                    static_cast<float>(maxDuration.count());
  ...
  std::cout << "Player " << getName() << " finished order ID "
            << currentOrder.getID() << " in " << elapsed.count()
            << " seconds, with TIPS: " << tipAmount << "( Price " << basePrice
            << " )\n";
  orders.pop();
}
