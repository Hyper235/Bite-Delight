...
void GrillStation::processAction(ActionType action,
  ...
  case ActionType::PutBurgerOnGrill:
    std::cout << "[GrillStation] Player " << player.getName()
              << " a pus carnea pe grill pt comanda #" << order.getID() << "\n";
    break;
  case ActionType::FlipBurger:
    std::cout << "[GrillStation] Player " << player.getName()
              << " a intors carnea pe grill pt comanda #" << order.getID()
              << "\n";
    break;
  case ActionType::TakeFromGrill:
    std::cout << "[GrillStation] Player " << player.getName()
              << " a luat carnea de pe grill pt comanda #" << order.getID()
              << "\n";
    break;
  default:
    break;
