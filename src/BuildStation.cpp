...
const FoodItem *
BuildStation::matchRecipe(const std::vector<Ingredient> &built) const {
...
void BuildStation::processAction(ActionType action, Ingredient *ingredient,
                                 Order &order,
  ...
  case ActionType::FinishItem: {
    ...
    if (!ok)
      throw ActionException(
          "This burger does not belong to the current order!");

    std::cout << "[BuildStation] Prepared item registered: " << built->getName()
              << "\n";
